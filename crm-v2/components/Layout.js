...
export default function Layout({ children }) {
  ...
  const [counts, setCounts] = useState({ contacts:0, problems:0, followups:0 })

  useEffect(() => {
    async function load() {
      const today = new Date().toISOString().split('T')[0]
      const [{ count:c },{ count:p },{ count:f }] = await Promise.all([
      ...
      setCounts({ contacts:c||0, problems:p||0, followups:f||0 })
    }
    load()
  }, [router.pathname])
